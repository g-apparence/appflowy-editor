...
class DocumentMarkdownDecoder extends Converter<String, Document> {
  ...
  List<Node> _parseRootNode(
    ...
    if (mdNode is! md.Element) {
      return _parseSingleNode(mdNode);
    } else {
      List<Node> tmpRes = [];
      for (final child in mdNode.children!) {
        tmpRes.addAll(_parseRootNode(child));
      }
      return tmpRes;
    }
  }

  List<Node> _parseSingleNode(md.Node mdNode) {
    List<Node> nodes = [];
...
// class DocumentMarkdownDecoderV1 extends Converter<String, Document> {
//   DocumentMarkdownDecoderV1({
